...
task_spawn(function()
    ...
    while not (GameActiveAbilityInstance and GameEquipFunction and HitscanFound) do
        local gc = getgc(true)
        for i = 1, #gc do
            if i % 5000 == 0 then task.wait() end -- Fragmenta a varredura para não congelar a CPU
            local item = gc[i]
        ...
        if not (GameActiveAbilityInstance and GameEquipFunction and HitscanFound) then task.wait(1) end
    end
end)
...
-- === MENTE 1: RASTREAMENTO COGNITIVO ===
            ...
            if not FriendCache[entity.Name] then
                ...
                if entityParent ~= workspace and entityParent ~= Entities then
                    if entityParent.ClassName == "Model" and entityParent:FindFirstChild("HumanoidRootPart") then
                        root = entityParent.HumanoidRootPart; realTargetModel = entityParent
                    end
                end
                if root then
...
UserInputService.InputBegan:Connect(function(input, processed)
    ...
    if input.KeyCode == Enum.KeyCode.R then
        ...
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "CHRONO QUANTUM V4",
            Text = IsSpamActive and "MÁXIMO PODER ATIVO" or "MOTOR: DESLIGADO",
            Duration = 1
        })
    end
end)local RANGE_LIMIT_SQ = RANGE_LIMIT * RANGE_LIMIT
