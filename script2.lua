...
task_spawn(function()
    ...
    while not (GameActiveAbilityInstance and GameEquipFunction and HitscanFound) do
        ...
        for i = 1, #gc do
            -- A cada 5000 itens verificados, damos uma pausa de 1 frame para a CPU respirar
            if i % 5000 == 0 then 
                task.wait() 
            end
...
-- === MENTE 2: MOTOR HIGH-FREQUENCY ADAPTATIVO (ESTÁVEL POR DELTATIME) ===
local networkTrigger = fireAbility or (AbilityActivated and AbilityActivated.FireServer)

local lastSendTime = 0
local ACCUMULATOR = 0
local TARGET_RATE = 1200 -- Distribuição cirúrgica de vazão
local TIME_STEP = 1 / TARGET_RATE
...
-- Cascades desenrolados de 5 pacotes em linha para força instantânea
local function executeBurstCascade(target)
...
-- GATILHO DE SIMULAÇÃO ADAPTATIVO
local function executeQuantumDischarge()
    ...
    if deltaTime > 0.1 then deltaTime = 0.016 end -- Anti-spike protection
    
    ACCUMULATOR = ACCUMULATOR + deltaTime

    -- Processamento síncrono baseado na taxa temporal exata da sua CPU
    while ACCUMULATOR >= TIME_STEP do
        ACCUMULATOR = ACCUMULATOR - TIME_STEP
        
        -- Onda 1: Rajada injetada direto na fila de frames
        task_spawn(executeBurstCascade, currentTarget)
        
        -- Onda 2: Adiantamento no agendador de tarefas nativo (Sem causar conflito de Actor)
        task.defer(fireNetworkOnly, currentTarget)
    end
end

-- PROPRIEDADE VISUAL TOTALMENTE ISOLADA (Anula disparos duplos de rede no PreRender)
...
RunService.Heartbeat:Connect(function()
    if IsSpamActive and CURRENT_RAW_TARGET and TARGET_BEST_PART then
        ...
        if localHrp then
            ...
            localHrp.AssemblyLinearVelocity = localHrp.AssemblyLinearVelocity + (direction * 0.05)
        end
    end
end)

-- === PIPELINE DE ENGENHARIA DE FLUXO CALIBRADO ===
RunService.PreRender:Connect(executeVisualOverdrive) -- Mantém os frames puros e fluidos
RunService.PreSimulation:Connect(executeQuantumDischarge) -- Disparos de rede regulados por tempo real
RunService.PostSimulation:Connect(executeQuantumDischarge)local RANGE_LIMIT_SQ = RANGE_LIMIT * RANGE_LIMIT
